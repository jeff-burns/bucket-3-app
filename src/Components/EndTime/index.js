...
const EndTime = (props) => {
  ...
  for (let i = 0; i < numTimes2; i++) {
      let timeStr2 = timeNow2.add(30, 'm').format('h:mm a')
      timesArray2.push(timeStr2);
  }
  const options2 = (
    <select className="custom-select options" id="end-time">
        <option value="">Pick End Time (REQUIRED)</option>
        {timesArray2.map(num => <option name="endTime" value={props.convertTime2}>{num}</option>)}
    </select>
  );
  return (
    <div>{options2}</div>
